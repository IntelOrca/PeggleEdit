...
namespace IntelOrca.PeggleEdit.Tools.Levels.Children
{
	...
	public class Movement : ICloneable, ILocation, IMovementContainer, ILevelChild
	{
		...
		public PegglePoint GetEstimatedMovePosition()
		{
			PegglePoint locationA = PegglePoint.Empty;
			if (mBaseMovement != null)
				locationA = mBaseMovement.GetEstimatedMovePosition();

			PegglePoint locationB = GetMyEstimatedMovePosition();

			return AddPoints(locationA, locationB);
		}

		public PegglePoint GetMyEstimatedMovePosition()
		{
			if (mTimePeriod == 0)
				return mAnchorPoint;
		...
		private PegglePoint GetAnchorPoint()
		{
			if (mBaseMovement != null) {
		...
		private PegglePoint AddPoints(PegglePoint a, PegglePoint b)
		{
			PegglePoint locationSum = new PegglePoint();
